...
def runUniformElection_SE(data, alpha, tracefile, batch = 1, init_batch = 1, a = 1, b = 1, t=0):
    ...
    while True:
        for c in range(C):
            ...
            if constiTerm > 0:
                ...
                const_sample[constituencies[c]] = sum(seenVotes[c])
                ...
                if term > 0:
                    ...
                    f.write(json.dumps(print_data) + '\n')
                    f.close()
                    with open('USE_{}.pickle'.format(t), 'wb') as pickle_file:
                        pickle.dump(const_sample, pickle_file)
