...
def Hoeffding(numberIterations, randomSeed, populationSize, partyDistribution, deltaValue):
	...
	for iter in range(numberIterations):
		...
		while(True):
			...
			for comm in range(numParties):
				...
				confidence_bounds[comm, 0] = max(empiricalMean[comm] - np.sqrt(beta/(2*samples)), 0)
				confidence_bounds[comm, 1] = min(empiricalMean[comm] + np.sqrt(beta/(2*samples)), 1)
			shouldExit = False
